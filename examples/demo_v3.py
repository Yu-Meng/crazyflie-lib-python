...
def drawCV(vs, out):
    def draw():
        classifier = cv2.CascadeClassifier("C:\\Program Files\\Opencv2.4.6\\opencv\\data\\haarcascades\\haarcascade_frontalface_default.xml")
        while(1):
            # ret, frame = cap.read()
            ret, frame = vs.read()
            # print('ret:{}',ret)
...
def selfcontrol():
    ...
    while True:
        ...
        if face_tracking_mode:
            ...
            if action_status == 0:
                if objectCX > 0 and objectCX < VIDEO_X_PIXEL * 0.4:
                    if action_status == 0:
                        mc.start_turn_left(rate=30)
                        action_status = 1
                if objectCX > VIDEO_X_PIXEL * 60 and objectCX <= VIDEO_X_PIXEL:
                    mc.start_turn_right(rate=30)
                    action_status = 2
            elif (objectCX >= VIDEO_X_PIXEL * 0.4 and objectCX <= VIDEO_X_PIXEL * 0.6) or (objectCX < 0):
                mc.stop()
                action_status = 0
            if objectCX < 0:
                if timing_status == 0:
...
if __name__ == '__main__':
    ...
    f = open('C:\\Users\\PCMYMAC\\Documents\\crazyflie\\Demooutput\\myfile.txt', 'w')
    ...
    ###multiple threads
    vs = VideoStream('http://192.168.10.116:8080/?action=stream').start()
    ...
    filename = 'E:\\Demooutput\\test_'+otherStyleTime + '.avi'
    fourcc = cv2.VideoWriter_fourcc(*'XVID')
    out = cv2.VideoWriter(filename, fourcc, 25, (VIDEO_X_PIXEL, VIDEO_Y_PIXEL))
    opencv_thread = drawCV(vs, out)
    opencv_thread.start()
    ...
    with SyncCrazyflie(URI) as scf:
                ...
                with MotionCommander(scf) as mc:
                   ...
                   #  time.sleep(20)
                    ...
                    #mc.down(0.5, velocity=0.3)
                    # time.sleep(2)
                    print("selfcontrol")
                    selfcontrol()

                    time.sleep(1)
                    mc.down(0.4, velocity=0.2)
